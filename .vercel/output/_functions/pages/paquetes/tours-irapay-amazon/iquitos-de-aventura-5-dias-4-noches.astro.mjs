...
const $$IquitosDeAventura = createComponent(($$result, $$props, $$slots) => {
  const tour = {
    image: "/../tours/irapay5d4n.avif",
    ...
    itinerary: [
      ...
      {
        day: "D\xEDa 4",
        title: "Tour Amanecer en el Amazonas, Observaci\xF3n de delfines, Tour Serpentario / Traslado a Iquitos",
      ...
      {
        day: "D\xEDa 5",
        title: "City Tours + Traslado al Aeropuerto",
        parrafo1: "Desayuno en el Hotel.",
        parrafo2: "Salimos 8 Am. Admirar\xE1n los hermosos mosaicos y balcones de hierro que fueron tra\xEDdos desde Europa para adornar los imponentes edificios que albergaban a los pr\xF3speros habitantes de la ciudad m\xE1s representativa de la Amazon\xEDa Peruana en un tiempo. Podr\xE1 apreciar la llamada \u201CCasa de Hierro\u201D, dise\xF1ada por el renombrado Gustave Eiffel y conocer la interesante historia de c\xF3mo finalmente fue edificada en la plaza principal de Iquitos. Conocer\xE1 el boulevard de la ciudad, lugar de reuni\xF3n para naturales y visitantes con su vista impresionante de los r\xEDos y la selva.",
        parrafo3: "Una parada en el mercado artesanal le permitir\xE1 ver y comprar artesan\xEDas coloridas hechas con materiales que se encuentran en el bosque circundante. Seguidamente visitaremos el Museo Amaz\xF3nico, Hist\xF3rico lugar construido en 1863, que fue ocupado por la Comandancia General del Ej\xE9rcito, y luego donado a la Prefectura de Loreto. Considerado Patrimonio Hist\xF3rico Cultural de la Naci\xF3n, restaurado en 1999, se convirti\xF3 en un importante museo de historia y cultura de la regi\xF3n. Exhibe una bella decoraci\xF3n mural en madera tallada y un conjunto de muebles antiguos. Lo m\xE1s impresionante del museo, es la colecci\xF3n de 80 esculturas hechas en fibra de vidrio con representaciones en tama\xF1o natural de las principales comunidades nativas del Amazonas del Per\xFA, Brasil y Venezuela. (Opcional Tour Quiticocha).",
        parrafo4: "Al finalizar el recorrido, lo trasladaremos a su hotel o al aeropuerto para abordar su vuelo de retorno."
      }
    ]
  };
  const relatedTours = [
...
<i class="fa-brands fa-whatsapp"></i> </a> </div> <div class="space-y-4"> ${tour.itinerary.map((item) => renderTemplate`<div class="bg-gray-50 p-2 rounded-lg shadow-sm"> <h2 class="text-lg sm:text-xl font-bold text-green-950 underline mb-3"> ${item.day}. ${item.title} </h2> <div class="space-y-4"> ${[
    ...
    item.parrafo2,
    item.parrafo3
  ].filter((parrafo) => parrafo).map((parrafo) => renderTemplate`<p class="text-gray-700 leading-relaxed">${parrafo}</p>`)} </div> </div>`)} </div> <div class="space-y-4"> ${accordionItems.map((item) => renderTemplate`<details class="bg-blue-50 rounded-lg shadow-sm p-4"> <summary class="text-lg sm:text-xl font-semibold cursor-pointer text-green-950"> ${item.title} </summary> <ul class="mt-2 space-y-1 text-gray-700"> ${item.content.map((text) => renderTemplate`<li class="ml-4 list-disc">${text}</li>`)} </ul> </details>`)} </div> <div class="flex flex-col"> <a href="https://wa.link/gfabsh" class="mt-2 w-full text-center bg-green-950 hover:bg-orange-600 transition-all py-2 px-4 text-white font-medium rounded-md" target="_blank">
Consultar ahora
...
}, "C:/astro/iquitos-expedition/src/components/IrapayAmazon/IquitosDeAventura.astro", void 0);

const $$IquitosDeAventura5Dias4Noches = createComponent(($$result, $$props, $$slots) => {
  return renderTemplate`${renderComponent($$result, "Layout", $$Layout, {}, { "default": ($$result2) => renderTemplate` ${renderComponent($$result2, "Navbar", $$Navbar, {})} ${renderComponent($$result2, "BreadcrumbIrapay", $$BreadcrumbIrapay, {})} ${renderComponent($$result2, "IquitosDeAventura", $$IquitosDeAventura, {})} ${renderComponent($$result2, "Footer", $$Footer, {})} ` })}`;
}, "C:/astro/iquitos-expedition/src/pages/paquetes/tours-irapay-amazon/iquitos-de-aventura-5-dias-4-noches.astro", void 0);

const $$file = "C:/astro/iquitos-expedition/src/pages/paquetes/tours-irapay-amazon/iquitos-de-aventura-5-dias-4-noches.astro";
const $$url = "/paquetes/tours-irapay-amazon/iquitos-de-aventura-5-dias-4-noches";

const _page = /*#__PURE__*/Object.freeze(/*#__PURE__*/Object.defineProperty({
    __proto__: null,
    default: $$IquitosDeAventura5Dias4Noches,
    file: $$file,
    url: $$url
